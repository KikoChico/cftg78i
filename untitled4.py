...
clock_image= ImageTk.PhotoImage(Image.open ("india map.jpg"))
#--------------India----------
...
japan_label.place(relx=0.7, rely=0.75, anchor=CENTER)

japan_clock=Label(root)
japan_clock["image"]=clock_image
japan_clock.place(relx=0.2,rely=0.55, anchor=CENTER)
...
class Japan():
    home=pytz.timezone('Japan/Japan Standard Time')
    local_time=datetime.now(home)
    current_time=local_time.strftime("%H:%M:%S")
    usa_time["text"]="Time :"+ current_time
...
japan_btn.place(relx=0.5,rely=0.8,anchor=CENTER)
root.mainloop()
